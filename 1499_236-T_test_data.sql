...
/* start test data for BUSINESS.CUST */
 ...
 inn_arr inn_arr_arrType := inn_arr_arrType(    -- array for variable test data
...
/* start test data for BUSINESS.ACCT */
 ...
 alt_acct_id_arr alt_acct_id_arrType := alt_acct_id_arrType(	--list of the account number linked to the client (order by prmry_cust_intrl_id)
...
/* service data for SQL-script */
 element_c INTEGER := 1;    -- loop counter for CUST
 element_a INTEGER := 1;    -- loop counter for ACCT
